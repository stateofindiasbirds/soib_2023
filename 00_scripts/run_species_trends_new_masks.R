...
load("01_analyses_states/Kerala/specieslists.RData")
lsa = specieslist %>% filter(!is.na(ht) | !is.na(rt))
listofspecies = c(lsa$COMMON.NAME,restrictedspecieslist$COMMON.NAME)
speclen = length(listofspecies)

dir.create("01_analyses_states/Kerala/trends/")
...
for (k in 1:200)
{
  ...
  nm = paste("/data",k,".csv",sep="")
  filename = paste("01_analyses_states/Kerala/dataforsim/",nm,sep = '')
  data = read.csv(filename)
  ...
  write.csv(trends, paste0('01_analyses_states/Kerala/trends/trends_', k,'.csv'), row.names = F)
