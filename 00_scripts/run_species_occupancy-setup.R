...
fullmap = fullmap %>%
  mutate(status = case_when(
    eBird.English.Name.2025 %in% doublespecies0 ~ "R",
    eBird.English.Name.2025 %in% doublespecies1 ~ "MS",
    eBird.English.Name.2025 %in% doublespecies2 ~ "MP",
    eBird.English.Name.2025 %in% doublespecies3 ~ "MP",
    TRUE ~ "M"
  ))

fullmap1 = fullmap %>% 
  filter(eBird.English.Name.2025 %in% doublespecies1) %>%
  mutate(status = "MW")
fullmap2 = fullmap %>% 
  filter(eBird.English.Name.2025 %in% doublespecies2) %>%
  mutate(status = "MS")
fullmap3 = fullmap %>% 
  filter(eBird.English.Name.2025 %in% doublespecies3) %>%
  mutate(status = "MW")
...
speciesforocc = fullmap %>%
  dplyr::select(eBird.English.Name.2025, status) %>%
  filter(eBird.English.Name.2025 %in% specieslist$COMMON.NAME)
noofspecies = length(speciesforocc$eBird.English.Name.2025)
