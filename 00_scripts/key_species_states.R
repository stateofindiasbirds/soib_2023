...
key.state.species0 = data0 %>%
  ...
  group_by(COMMON.NAME) %>% mutate(tot.range = n_distinct(gridg1)) %>%
  group_by(ST_NM,COMMON.NAME) %>% reframe(prop.range = n_distinct(gridg1)/max(tot.range)) %>%
  left_join(main, by = c("COMMON.NAME" = "eBird.English.Name.2022")) %>%
  mutate(
    Current.Sort = case_when(
      SOIBv2.Current.Status %in% c("Insufficient Data","Trend Inconclusive") ~ 1,
      TRUE ~ 0),
    
    Long.Term.Sort = case_when(
      SOIBv2.Long.Term.Status %in% c("Insufficient Data","Trend Inconclusive") ~ 1,
      TRUE ~ 0)) %>%
  group_by(ST_NM) %>% arrange(ST_NM,desc(prop.range),Current.Sort,Long.Term.Sort,rangemean) %>%
  dplyr::select(ST_NM,COMMON.NAME,prop.range,SOIBv2.Current.Status,Current.Sort,SOIBv2.Long.Term.Status,Long.Term.Sort,rangemean)
...
key.state.species.report = key.state.species.trends %>%
  ...
  left_join(IUCN.flag) %>% arrange(desc(prop.range),.by_group=TRUE) %>%
  group_by(ST_NM) %>% slice(1:max(flag)) %>% ungroup() %>%
  select(-flag)
to.rem = key.state.species.report %>%
  distinct(ST_NM,COMMON.NAME)
IUCN.add = key.state.species %>%
  filter(ST_NM %in% IUCN.end.st & COMMON.NAME %in% IUCN.end.species) %>%
  anti_join(to.rem) %>%
  group_by(ST_NM) %>% arrange(desc(prop.range),.by_group=TRUE) %>%
  slice(1)
...
check = key.state.species.report %>%
  group_by(ST_NM) %>% reframe(n = n()) %>%
  filter(n < 4)

key.state.species.trends.extra = key.state.species %>%
  filter(ST_NM %in% check$ST_NM) %>%
  left_join(check) %>% mutate(n = 4 - n + 1) %>%
  filter(Long.Term.Sort == 0 | Current.Sort == 0) %>%
  group_by(ST_NM) %>% arrange(desc(prop.range),.by_group=TRUE) %>%
  slice(2:max(n)) %>% ungroup() %>% select(-n)
...
IUCN.add = key.state.species %>%
  ...
  slice(1)
key.state.species.report = key.state.species.report %>%
  bind_rows(IUCN.add) %>% group_by(ST_NM) %>% arrange(desc(prop.range),.by_group=TRUE)
...
check1 = key.state.species.report %>%
  filter(!COMMON.NAME %in% IUCN.end.species) %>%
  group_by(COMMON.NAME) %>% reframe(n = n(), p = min(prop.range)) %>% filter(n > 1 & p < 0.35) %>%
  distinct(COMMON.NAME) %>% as.vector() %>% list_c()
check0 = check1
...
for(i in 1:10)
{
  comb.to.remove = key.state.species.report %>%
    ...
    group_by(COMMON.NAME) %>% arrange(prop.range, .by_group=TRUE) %>% slice(1) %>%
    distinct(ST_NM,COMMON.NAME)
  ...
  if (length(check$ST_NM) > 0)
  {
    key.state.species.extra = key.state.species %>%
      ...
      filter(ST_NM %in% check$ST_NM) %>%
      left_join(check) %>% mutate(n = 4 - n + i - 1) %>%
      group_by(ST_NM) %>% arrange(desc(prop.range),.by_group=TRUE) %>%
      slice(i:max(n)) %>% ungroup() %>% select(-n)
...
key.state.species.extra = key.state.species %>%
  anti_join(combs.to.avoid) %>%
  filter(ST_NM %in% check$ST_NM) %>%
  left_join(check) %>% mutate(n = 4 - n) %>%
  group_by(ST_NM) %>% arrange(desc(prop.range),.by_group=TRUE) %>%
  slice(1:max(n)) %>% ungroup() %>% select(-n)
...
key.state.species.report.temp = key.state.species.report %>%
  filter(prop.range < 0.08, ST_NM %in% initial.report.range$ST_NM) %>%
  group_by(ST_NM) %>% arrange(prop.range, .by_group=TRUE) %>% slice(1) %>%
  distinct(ST_NM,COMMON.NAME)
...
initial.report.range.temp = initial.report.range %>% 
  ...
  group_by(ST_NM) %>% slice(1:max(n)) %>% dplyr::select(-n)

key.state.species.report = key.state.species.report %>%
  anti_join(key.state.species.report.temp) %>%
  bind_rows(initial.report.range.temp) %>%
  group_by(ST_NM) %>% arrange(desc(prop.range), .by_group=TRUE) %>% ungroup()
...
initial.report.rem = initial.report %>%
  ...
  group_by(ST_NM) %>% arrange(desc(prop.range), .by_group=TRUE) %>% slice(1:max(n)) %>%
  select(-n)

key.state.species.report = key.state.species.report %>%
  bind_rows(initial.report.rem) %>%
  group_by(ST_NM) %>% arrange(desc(prop.range), .by_group=TRUE) %>% slice(1:4) %>%
  rename(eBird.English.Name.2022 = COMMON.NAME)

miss.spec = data.frame(ST_NM = c("Manipur","Andhra Pradesh","Uttarakhand","Himachal Pradesh",
                                 "Gujarat","Kerala","Andaman and Nicobar Islands","Bihar",
                                 "Maharashtra","West Bengal","Assam","Assam","Goa"),
                       eBird.English.Name.2022 = c("Manipur Bush-Quail","Jerdon's Courser","Finn's Weaver",
                                 "Ruddy Shelduck","Greater Flamingo","Garganey",
                                 "Nicobar Megapode","Northern Pintail","Indian Courser",
                                 "Rufous-necked Hornbill","White-winged Duck",
                                 "Bengal Florican","Black-capped Kingfisher"))

add.final = main %>% filter(eBird.English.Name.2022 %in% c("Manipur Bush-Quail","Jerdon's Courser","Finn's Weaver",
                                                           "Ruddy Shelduck","Greater Flamingo","Garganey",
                                                           "Nicobar Megapode","Northern Pintail","Indian Courser",
                                                           "Rufous-necked Hornbill","White-winged Duck",
                                                           "Bengal Florican","Black-capped Kingfisher")) %>%
  left_join(miss.spec) %>%
  dplyr::select(ST_NM,eBird.English.Name.2022,SOIBv2.Current.Status,SOIBv2.Long.Term.Status,SOIBv2.Range.Status)

key.state.temp = key.state.species %>% dplyr::select(ST_NM,COMMON.NAME,prop.range) %>%
  rename(eBird.English.Name.2022 = COMMON.NAME)
  
prop.to.add = add.final %>%
  left_join(key.state.temp) %>%
  dplyr::select(ST_NM,eBird.English.Name.2022,prop.range,SOIBv2.Current.Status,SOIBv2.Long.Term.Status,SOIBv2.Range.Status)
...
one.perc = main %>% filter(!is.na(Onepercent.Estimates)) %>%
...
full.list.one.perc = data0 %>%
  ...
  filter(year > 2017) %>%
  left_join(one.perc) %>%
  filter(!is.na(Onepercent.Estimates),OBSERVATION.COUNT != "X") %>%
  mutate(OBSERVATION.COUNT = as.numeric(OBSERVATION.COUNT)) %>%
  filter(OBSERVATION.COUNT >= Onepercent.Estimates, eBird.English.Name.2022 %in% specs.states) %>%
...
full.list.prop.rem = data0 %>%
  ...
  filter(year > 2017) %>%
  filter(eBird.English.Name.2022 %in% specs.states) %>%
  group_by(eBird.English.Name.2022) %>% mutate(tot.range = n_distinct(gridg1)) %>%
  group_by(ST_NM,eBird.English.Name.2022) %>% reframe(prop.range = n_distinct(gridg1)/max(tot.range)) %>%
...
write.csv(delim.state.list, "01_analyses_full/key_state_species_full_list_delim.csv", row.names = F)
