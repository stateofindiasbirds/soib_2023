...
trends_full = read.csv("assorted_trends_4.csv")
indtrends_full = read.csv("Vembanad_AWC.csv")
scol = "#869B27"


for (i in unique(trends_full$species))
{
  trends = trends_full %>% filter(species == i)
  indtrends = indtrends_full %>% filter(species == i)
  ...
  if (limlt < 0)
    limlt = 0
  ybreaks = seq(limlt,limu,length.out=5)
  ybreaksl = round(ybreaks)
  ...
  ggpx2 = ggdraw(ggpx) +
    draw_grob(rect1) + draw_grob(rect2) + draw_grob(rect3) + draw_grob(rect4) +
    draw_grob(rect5) + draw_grob(rect6) + draw_grob(rect7) +
    draw_grob(rect8) + draw_grob(rect9)
  
  g1 = plot_grid(ggpx2,ggpx1,nrow=2,ncol=1,rel_widths = c(1/2, 1/2))
  
  sps = as.character(trends$species[1])
  name = paste(sps,"_","independent_soib_comparison.jpg",sep="")
